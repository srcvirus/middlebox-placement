...
2. Removed dummy servers for deploying ingress/egress, they r deployed on the actual servers
  ...
  //for (int time_limit = 100; time_limit < 180; time_limit += 10) {
    ...
    while (max_vnf != last_max_vnf) {
      ...
      run_cplex_opt(traffic_requests, opex, opex_breakdown, running_time, 
        sequence, path, all_edges, delays, utilization, topology_filename, 
        max_vnf, is_feasible, time_limit);
      if (is_feasible) {
    ...
    if (atleast_one_solution_found) {
      ...
      printf("TrafficTimeIns %d TimeLimit %d #VNF %d Runtime %llu.%llu\n", 
        traffic_requests[0].arrival_time, time_limit, max_vnf, 
        solution_time / ONE_GIG, solution_time % ONE_GIG);

      break; // from the for loop for time limit
    }
  }
...
void run_cplex_opt(std::vector<traffic_request> traffic_requests, 
  ...
  try {
        ...
        for (int n2 : nbr[t][n1]) {
          if (n1 < n2) {
            for (int _u = 0; _u < kSwitchCount; ++_u) {
              ...
              // the following constraint is for consecutive middleboxed attached to the same switch
              model.add(wtuv_u_v[t][n1][n2][_u][_u] >= ztn_n[t][n1][_u] + ztn_n[t][n2][_u] - 1);
            }
          }
        }
      }
    ...
    for (int t = 0; t < kTrafficCount; ++t) {
      for (int n1 = 0; n1 < trafficNodeCount[t]; ++n1) {
        for (int n2 : nbr[t][n1]) {
          ...
          model.add(sum > 0);
          model.add(IloIfThen(env, normal_edge_sum == 0, 0 <= self_loop_sum <= 1)); // at most one self-loop for a single link
        }
      }
    }
    //---------------------------------------------------------------------
    ...
    */
    ...
    //-----CPLEX Constraint------------------------------------------------
    IloExpr maxvnfCount(env);
    for (int m = 0; m < kMboxCount; ++m) {
      maxvnfCount += ym[m];
    }
    model.add(maxvnfCount <= max_vnf);
    //---------------------------------------------------------------------
        ...
        for (int n2 : nbr[t][n1]) {
          if (n1 < n2) {
            for (int _u = 0; _u < kSwitchCount; ++_u) {
              for (int _v : _nbr[_u]) {
                if (_u < _v) {
                  forwardingCost += 0.001 * ( wtuv_u_v[t][n1][n2][_u][_v] + wtuv_u_v[t][n1][n2][_v][_u] ) * 
                                            beta_t * per_bit_transit_cost * traffic_requests[t].duration;
                }
              }
        ...
        for (int n2 : nbr[t][n1]) {
          if (n1 < n2) {
            ...
            for (int _u = 0; _u < kSwitchCount; ++_u) {
              for (int _v : _nbr[_u]) {
                if (_u < _v) {
                  delay += (wtuv_u_v[t][n1][n2][_u][_v] * _delta[_u][_v] + wtuv_u_v[t][n1][n2][_v][_u] * _delta[_v][_u]);
                }
              }
            }
          }
        ...
        for (int n2 : nbr[t][n1]) {
          if (n1 < n2) {
            for (int _u = 0; _u < kSwitchCount; ++_u) {
              for (int _v : _nbr[_u]) {
                if (_u < _v) {
                  value = cplex.getValue(wtuv_u_v[t][n1][n2][_u][_v]) + cplex.getValue(wtuv_u_v[t][n1][n2][_v][_u]);
        ...
        for (int n2 : nbr[t][n1]) {
          if (n1 < n2) {
            for (int _u = 0; _u < kSwitchCount; ++_u) {
              for (int _v : _nbr[_u]) {
                if (_u < _v) {
                  delay += ( cplex.getValue(wtuv_u_v[t][n1][n2][_u][_v]) + cplex.getValue(wtuv_u_v[t][n1][n2][_v][_u]) ) * _delta[_u][_v];
                }
              }
            }
          }
