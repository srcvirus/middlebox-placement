  ...
  try {
      ...
      for (int _n = 0; _n < kServerCount; ++_n) {
        if (dp_n[p][_n] == 1) {
          if (p == 0 || p == 1) {
            ...
            // mcount = floor(c_nr[_n][0] * 1.0 / middleboxes[p -
            // 2].cpu_requirement);
            mcount = 1;
    ...
    */
    ...
    for (int m = 0; m < kMboxCount; ++m) {
      ...
      energyCost += HW_MBOX_POWER_CONSUMPTION(traffic_load * 0.001) *
...
// turn-off console output for cplex
    ...
    const IloNum relativeGap =
        0.01;  // find Integer solution within 1% of optimal
  ...
  std:
    ...
    for (int m = 0; m < kMboxCount; ++m) {
      ...
      for (int t = 0; t < kTrafficCount; ++t) {
        for (int n = 0; n < trafficNodeCount[t]; ++n) {
          traffic_load +=
              cplex.getValue(xtnm[t][n][m]) * traffic_requests[t].min_bandwidth;
        }
      }
      DEBUG("Middlebox %d processed %lf kbps traffic\n", m, traffic_load);
      enrCost += HW_MBOX_POWER_CONSUMPTION(traffic_load * 0.001) *
                 duration_hours * PER_UNIT_ENERGY_PRICE;
    }
