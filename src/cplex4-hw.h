  ...
  try {
    ...
    */
    ...
    for (int m = 0; m < kMboxCount; ++m) {
      ...
      IloExpr traffic_load(env);
      for (int t = 0; t < kTrafficCount; ++t) {
        for (int n = 0; n < trafficNodeCount[t]; ++n) {
          traffic_load += xtnm[t][n][m] * traffic_requests[t].min_bandwidth;
        }
      }
      energyCost += HW_MBOX_POWER_CONSUMPTION(traffic_load * 0.001) * duration_hours * PER_UNIT_ENERGY_PRICE;
    }
    /*
    for (int _n = 0; _n < kServerCount; ++_n) {
    ...
    */
    ...
    for (int m = 0, sw; m < kMboxCount; ++m) {
      if (fabs(ym_vals[m] - 1) < EPS) {
        ...
        //sw = (server4mbox[m] < kInitialSwitchCount) ? server4mbox[m] : (server4mbox[m] - kInitialSwitchCount);
    ...
    for (int m = 0; m < kMboxCount; ++m) {
      ...
      double traffic_load = 0.0;
      for (int t = 0; t < kTrafficCount; ++t) {
        for (int n = 0; n < trafficNodeCount[t]; ++n) {
          traffic_load += cplex.getValue(xtnm[t][n][m]) * traffic_requests[t].min_bandwidth;
        }
      }
      DEBUG("Middlebox %d processed %lf kbps traffic\n", m, traffic_load);
      enrCost += HW_MBOX_POWER_CONSUMPTION(traffic_load * 0.001) * duration_hours * PER_UNIT_ENERGY_PRICE;
    }
        ...
        for (int n2 : nbr[t][n1]) {
          if (n1 < n2) {
            for (int _u = 0; _u < kInitialSwitchCount; ++_u) {
              for (int _v : _nbr[_u]) {
                if (_u < _v) {
                  delay += ( cplex.getValue(wtuv_u_v[t][n1][n2][_u][_v]) + cplex.getValue(wtuv_u_v[t][n1][n2][_v][_u]) ) * delta_u_v[_u][_v];
                }
              }
            }
          }
