...
1. Implementation of the puzzle paper
2. chnaged the objective function and added a constraint for max number of vnfs
...
void run_cplex_opt(std::vector<traffic_request> traffic_requests, double &opex,
                   ...
                   int *delays, std::vector<int> &utilization,
                   string topology_filename, int max_vnf, int &is_feasible);
...
void run_cplex(std::vector<traffic_request> traffic_requests, double &opex,
  ...
  while (max_vnf != last_max_vnf) {
    ...
    run_cplex_opt(traffic_requests, opex, opex_breakdown, running_time,
                  ...
                  topology_filename, max_vnf, is_feasible);
    if (is_feasible) {
  ...
  printf("time_instance %d vnf_count %d running_time %llu.%llu\n",
         traffic_requests[0].arrival_time, max_vnf, solution_time / ONE_GIG,
         solution_time % ONE_GIG);
}
...
void run_cplex_opt(std::vector<traffic_request> traffic_requests, double &opex,
  ...
  try {
    ...
    int kSwitchCount = 0, kInitialSwitchCount = 0, kLinkCount = 0,
        kServerCount = 0, kResourceCount = 0;
    ...
    */
    ...
    int seed =
        kSwitchCount;  // this is the starting number for the new switches
    ...
    for (int _u = 0; _u < kSwitchCount; ++_u) {
      for (int _v : __nbr[_u]) {
        if (_u < _v) {
          ...
          // cout << "_u _v " << _u << " " << _v << " " << beta_u_v[_u][_v] <<
          // endl;
    ...
    */
    ...
    /*
        ...
        //IloNum duration_hours = 1.0 * traffic_requests[0].duration / (60.0 *
       60.0);
       ...
       + wtuv_u_v[t][n1][n2][_v][_u] * delta_u_v[_v][_u]);
          ...
          penalty += ( (delay - tr.max_delay) + IloAbs(delay - tr.max_delay)
       )/2.0  * tr.delay_penalty;
        }
    ...
    */
    ...
    // build the objective
    IloExpr objective(env);
    // add number of middleboxes
    IloExpr vnfCount(env);
    for (int m = 0; m < kMboxCount; ++m) {
      vnfCount += ym[m];
    }
    objective += vnfCount;
...
#endif
    // set time limit
    const IloInt timeLimit = 20;  // in seconds
  ...
  std:
    ...
    for (int m = 0, sw; m < kMboxCount; ++m) {
      if (fabs(ym_vals[m] - 1) < EPS) {
        ...
        // sw = (server4mbox[m] < kInitialSwitchCount) ? server4mbox[m] :
        // (server4mbox[m] - kInitialSwitchCount);
        sw = pseudo2actual[switch4mbox[m]];
          ...
          if (n1 < n2) {
            for (int _u = 0; _u < kSwitchCount; ++_u) {
              ...
              for (int _v : __nbr[_u]) {
                ...
                if (fabs(value - 1) < EPS) {
                  // DEBUG("Traffic %d link (%d, %d) mapped to phy. link (%d,
                  // %d)\n", t, n1, n2, _u, _v);
                  if (_u < kInitialSwitchCount && _v < kInitialSwitchCount) {
                    DEBUG(
                        "Traffic %d link (%d, %d) mapped to phy. link (%d, "
                        "%d)\n",
                        t, n1, n2, _u, _v);
    ...
    for (int _n = kInitialSwitchCount, consumed_cores; _n < kServerCount;
         ++_n) {
      consumed_cores = 0;
      for (int m : mbox4server[_n]) {
