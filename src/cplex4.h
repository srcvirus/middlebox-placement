...
void print_IloInt2dArray(IloInt2dArray a, int dimension1, int dimension2, string name) {
  cout << name << endl;
...
void print_IloInt3dArray(IloInt3dArray a, int dimension1, int dimension2, int dimension3, string name) {
  cout << name << endl;
  for (int i = 0; i < dimension1; ++i) {
    cout << "dim1 = " << i << endl;
    for (int j = 0; j < dimension2; ++j) {
...
void run_cplex(std::vector<traffic_request> traffic_requests, double &opex, double &running_time){
    ...
    try{
        ...
        //ingress and egress middleboxes are deployed on special non-existent servers
        kServerCount += kSwitchCount; //these are the special ones
        kServerCount += kSwitchCount; //TODO: read from file, current assumption: there is one server per switch
        ...
        //TODO: need to change the topology file to include information about servers
        ...
        for (int i = 0, sw, cpu; i < kSwitchCount; ++i) {
          ...
          _z_s_n[sw][sw+kSwitchCount] = 1; // these is where we deploy normal middleboxes
        ...
        // middlebox type 0 & 1 can be deployed only on server [0...(kSwitchCount-1)]
        for (int p = 0; p < 2; ++p) {
        ...
        //declare CPLEX varibales beta_u_v and delta_u_v, ### do not move, kSwitchCount must be @ new value
        ...
        std::vector<int> mbox4switch[kSwitchCount]; //do not move, kSwitchCount must be @ new value
        for (int _n = 0; _n < kServerCount; ++_n) {
        ...
        */
        ...
        // we cannot uses amp in the previous steps as the total count of middleboxes is not known 
        ...
        */
        ...
        //DECISION VAR: ftl_l = R+, amount of bandwidth allocate to link l of traffic t on phy. link _l
        ...
        //DECISION VAR: htl_l = 1, if link l of traffic t provisioned on  phy. link _l
        ...
        //      model.add(IloIfThen(env, (ftl_l[t][l][_l] > 0), (htl_l[t][l][_l] == 1)));
        //      model.add(IloIfThen(env, (htl_l[t][l][_l] == 1), (ftl_l[t][l][_l] > 0)));
        //    }
        //  }
        //}
        ...
        for (int t = 0, u_t, v_t; t < kTrafficCount; ++t) {
          ...
          for(int m = 0; m < kMboxCount; ++m) {
            model.add(IloIfThen(env, xtnm[t][0][m] == 1, ym[m] * _z_s_n[u_t][server4mbox[m]] == 1));
            model.add(IloIfThen(env, xtnm[t][trafficNodeCount[t]-1][m] == 1, ym[m] * _z_s_n[v_t][server4mbox[m]] == 1));
          }
        }
        //---------------------------------------------------------------------

        //^^^^^CPLEX Decision Variable^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        //wtuv_u_v = 1, if logical link (u, v) of traffic t uses physical link (_u, _v)
            ...
            if (_u < _v) {
              ...
              for (int t = 0, beta_t; t < kTrafficCount; ++t) {
                ...
                for (int n1 = 0; n1 < trafficNodeCount[t]; ++n1) {
                  for (int n2 : nbr[t][n1]) {
                    if (n1 < n2) {
                      sum += (wtuv_u_v[t][n1][n2][_u][_v] + wtuv_u_v[t][n1][n2][_v][_u]) * beta_t;
                    }
                  }
                }
              }
          ...
          for (int n1 = 0; n1 < trafficNodeCount[t]; ++n1) {
            for (int n2 : nbr[t][n1]) {
              if (n1 < n2) {
                ...
                model.add(sum > 0);
                for (int _s = 0; _s < kSwitchCount; ++_s) {
                  //model.add(IloIfThen(env, ztn_n[t][n1][_s] != ztn_n[t][n2][_s], sum > 0));
                  //model.add(IloIfThen(env, ztn_n[t][n1][_s] == ztn_n[t][n2][_s], wtuv_u_v[t][n1][n2][_s][_s] == 1));
                }
              }
            }
          }
        ...
        for (int t = 0; t < kTrafficCount; ++t) {
          for(int n = 0; n < trafficNodeCount[t]; ++n) {
            for (int m = 0; m < kMboxCount; ++m) {
              ...
              //model.add(IloIfThen(env, xtnm[t][n][m] == 1, sum_mbox == 1 && sum_type == 1));
              model.add(IloIfThen(env, xtnm[t][n][m] == 1, sum_type == 1));
              //cnst.add(IloIfThen(env, xtnm[t][n][m] == 1, sum_mbox == 1 && sum_type == 1));
            }
          }
        }
        //---------------------------------------------------------------------
        ...
        */
          ...
          for(int n1 = 0; n1 < trafficNodeCount[t]; ++n1) {
            for (int n2 : nbr[t][n1]) {
              if (n1 < n2) {
                ...
                for (int _u = 0; _u < kSwitchCount; ++_u) {
                  for(int _v : __nbr[_u]) {
                    delay += wtuv_u_v[t][n1][n2][_u][_v] * delta_u_v[_u][_v] + wtuv_u_v[t][n1][n2][_v][_u] * delta_u_v[_v][_u];
                  }
                }
        ...
        for (int m = 0, sw; m < kMboxCount; ++m) {
          if (ym_vals[m] == 1) {
            ...
            sw = (server4mbox[m] < kSwitchCount)?server4mbox[m]:(server4mbox[m]-kSwitchCount);
            cout << "Middlebox " << m << " (" << type << ") is active on server " << sw << endl;
          }
        } 
          ...
          for (int n = 0; n < trafficNodeCount[t]; ++n) {
            ...
            for (int _s = 0; _s < kSwitchCount; ++_s) {
              if (ztn_n_vals[_s] == 1) {
                ...
                cout << "Traffic " << t << " node " << n << " provisioned on switch " << pseudo2actual[_s] << " pseudo-switch " << _s << endl;
              }
            }
          }
        }
              ...
              if (n1 < n2) {
                for (int _u = 0; _u < kSwitchCount; ++_u) {
                  ...
                  for (int _v : __nbr[_u]) {
                    ...
                    if (value == 1) {
                      cout << "Traffic link (" << n1 << ", " << n2 << ") mapped to phy. link (" << _u << ", " << _v << ")" << endl;
                    }
                  }
                  //value = cplex.getValue(wtuv_u_v[t][n1][n2][_u][_u]);
                  //if (value == 1) {
                  //  cout << "Traffic link (" << n1 << ", " << n2 << ") mapped to phy. link (" << _u << ", " << _u << ")" << endl;
                  //}
                }
              }
            }
        ...
        for (int t = 0; t < kTrafficCount; ++t) {
          for (int l = 0; l < trafficLinkCount[t]; ++l) {
            ...
            for (int _l = 0; _l < kLinkCount; ++_l) {
              if (ftl_l_vals[_l] > 0) {
                cout << "Traffic " << t << " link " << l << " bandwidth on phy. link " << _l << " " << ftl_l_vals[_l]  << endl;
              }
            }
          }
        }
        ...
        for (int t = 0; t < kTrafficCount; ++t) {
          for (int l = 0; l < trafficLinkCount[t]; ++l) {
            ...
            for (int _l = 0; _l < kLinkCount; ++_l) {
              if (htl_l_vals[_l] == 1) {
                cout << "Traffic " << t << " link " << l << " provisioned on phy. link " << _l << endl;
              }
            }
          }
        } 
